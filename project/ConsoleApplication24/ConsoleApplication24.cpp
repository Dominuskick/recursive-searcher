...
namespace MCConcurrency {
	inline int threadsCount(int iterationsCount = -1) {
		...
		if (numThreads == 0) numThreads = DEF_THREAD_COUNT;
		if (iterationsCount == 0) return 1;
	...
	void parellel_for(int first, int last, int threadsNumbers, const Function& I) {
		...
		const auto chunk = std::max(static_cast<int>(std::ceil(elementsCount / static_cast<double>(numThreads))), 1);
...
int main() {
	...
	getchar();
	auto t0 = Time::now();
	MCConcurrency::parellel_for(0, N, numThreads, [&](int nIndex, int threadNumber) {
